...
Use the canonical domain name from the 17-domain map:

```
identity | tenancy | authz | crm | channels | conversations | routing |
ai-runtime | knowledge | actions | orders | reservations | cases |
approvals | audit | billing | analytics
```

For cross-cutting or infrastructure changes, use: `infra`, `deps`, `ci`.
...
feat(ai-runtime): add OpenAI provider abstraction
feat(knowledge): add prompt template versioning
...
1. **One domain per commit.** One commit should belong to one clear domain or one closely related integration point.
...
4. **Explicit migrations.** Migration commits must be explicit: `feat(<domain>): add <table> migration`.
